...
\subsubsection{compiler.py}

\subsubsection{compileAndLoad.py}
Kör kompilatorn och ersätter programminnet i cpu.vhd med det nya programmet. Sätter även en begränsning för när PM ska laddas med information från programminnet. Krävs för att det inte ska gå out of range när ASR sätts till något högre än storleken på \texttt{p\_mem}. Används inte \texttt{compileAndLoad.py} så bör denna begränsning sättas för hand.
...
\noindent
Sli och include är speciella, och har inte samma syntax som resten av instruktionerna; båda tar endast ett argument, och har varken support för moder, labels, eller indikatorer. Detta då de körs i kompileringstid, snarare än som maskinkod.
...
Pythonscriptet compileAndLoad.py kan dock göra detta åt dig; den kompilerar en fil och skriver sedan in resultatet i cpu.vhd. 

Notera att detta är i allra högsta grad specialanpassad för detta projekt, så omstrukturering av mappar, eller större ändringar i cpu.vhd kan få scriptet att sluta funka. Körs via:\\
